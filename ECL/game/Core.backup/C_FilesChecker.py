...
logger = get_logger("Files_Checker")
...
class FilesChecker:
    ...
    def __find_api(self, get_url: str, get_path: str) -> str:  # 根据URL判断下载源
    ...
    def __check_game_jar(self, game_path: Path, version_name: str, version_json: dict) -> list[tuple[str, str]]:  # 检查游戏本体完整性
        download_list = []
    ...
    def __check_libraries(self, game_path: Path, version_json: dict) -> list[tuple[str, str]]:  # 检查依赖库完整性
        download_list = []
        self.output_log("检查依赖库完整性...")
        for libraries in version_json.get("libraries"):  # 遍历libraries
    ...
    def __check_assets(self, game_path: Path, version_json: dict) -> list[tuple[str, str]]:  # 检查资源文件完整性
        download_list = []
        ...
        if C_Libs.get_file_sha1(asset_index_path) != index_file_sha1:  # 下载资源索引
            try:
    ...
    def check_files(self, game_path: str | Path, version_name: str, download_max_thread: int):  # 主入口:检查并补全文件
